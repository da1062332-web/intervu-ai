...
test.describe.serial("Module 1 Cross-Module Validation E2E Scenarios", () => {
  ...
  test("Scenario 1: Create Config -> Create Section -> Assign Topic -> Assign Weightage -> Config & Knowledge PASS", async ({
    page,
  }) => {
  ...
  test("Scenario 2: Create Template -> Create Variable -> Create Rule -> Validate Template PASS", async ({
    page,
  }) => {
  ...
  test("Scenario 3 & 4: Create Blueprint -> Assign Style Profile -> Validate Blueprint PASS -> score = 100 (READY)", async ({
    page,
  }) => {
    ...
    const sectionsJson = [
      {
        ...
        topicAllocations: [{ topicId: topicId, percentage: 100 }],
      },
    ];
