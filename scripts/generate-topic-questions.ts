...
async function generateQuestionsForTopic(
  ...
  try {
    ...
    if (templates.length === 0) {
      ...
      console.log(
        `Hint: Make sure templates have their conceptKey set to one of: ${conceptCodes.join(", ")}`,
      );
      return;
    }

    console.log(
      `Found ${templates.length} templates for this topic. Generating ${countPerTemplate} questions per template...`,
    );
    ...
    for (const template of templates) {
      console.log(
        `Generating for template: ${template.templateKey} (Difficulty: ${template.difficultyLevel})`,
      );
      ...
      for (let i = 0; i < countPerTemplate; i++) {
        try {
          ...
          if (result && result.validation && result.validation.isValid) {
            ...
            generatedCount++;
            console.log(
              `  - Success: Generated question ${i + 1}/${countPerTemplate}`,
            );
          } else {
            failedCount++;
            console.error(
              `  - Failed: Validation failed for question ${i + 1}/${countPerTemplate}`,
            );
          }
        } catch (err: any) {
          failedCount++;
    ...
    if (generatedCount > 0) {
      console.log(
        `You can now go back to the UI and assign topic '${topic.name}' to the section.`,
      );
    }
  } catch (error) {
