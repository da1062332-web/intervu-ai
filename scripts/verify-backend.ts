...
async function run() {
  ...
  try {
    ...
    try {
      const outputStr = execSync("npx tsx scripts/verify-migrations.ts", { encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"] });
    ...
    try {
      const outputStr = execSync("npx tsx scripts/verify-database.ts", { encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"] });
    ...
    try {
      const outputStr = execSync("npx tsx scripts/verify-transactions.ts", { encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"] });
    ...
    try {
      ...
      if (rlsEnabled) {
        ...
        securityPassed = true;
        summary.details.security = { rls: "ENABLED", tables: rlsStatus.filter((t) => t.rowsecurity) };
      } else {
        ...
        summary.details.security = { rls: "NOT APPLICABLE", info: "Row Level Security is not enabled on Postgres tables. Isolation managed at App/Guards layer." };
      }
    } catch (err: any) {
    ...
    try {
      ...
      await prisma.recommendation.deleteMany({ where: { evaluation: { userId: { startsWith: "perf-user-id-" } } } });
      await prisma.skillScore.deleteMany({ where: { evaluation: { userId: { startsWith: "perf-user-id-" } } } });
      await prisma.evaluationResult.deleteMany({ where: { userId: { startsWith: "perf-user-id-" } } });
      await prisma.performanceSummary.deleteMany({ where: { userId: { startsWith: "perf-user-id-" } } });
      await prisma.testInstance.deleteMany({ where: { userId: { startsWith: "perf-user-id-" } } });
      await prisma.user.deleteMany({ where: { id: { startsWith: "perf-user-id-" } } });
    ...
    const overallPassed = migrationPassed && repositoryPassed && transactionPassed && securityPassed && performancePassed;
    ...
    return {
      ...
      data: summary,
      error: overallPassed ? null : { code: "LAUNCH_CERTIFICATION_FAILED", message: "One or more launch readiness checks failed." },
