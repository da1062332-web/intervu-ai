...
async function runVerification() {
  console.log(
    "🚀 Starting End-to-End Dataset Flow Verification (question-generation)...",
  );
  ...
  const templateDatasetService = new TemplateDatasetService(
    templateDatasetRepo,
    templateRepo,
  );
  ...
  const strategyResolver = new GenerationStrategyResolver(
    prisma,
    strategyRegistry,
  );
  ...
  promptTemplateRegistry.register(
    "DATASET",
    "Generate question based on {{passage}} testing topic {{topic}} and company {{company_name}} and role {{role}}",
  );
  ...
  console.log(
    "✅ Dataset schema inferred successfully:",
  ...
  const savedConfig = await templateDatasetService.saveDatasetConfig(
    templateId,
    mappingDto,
  );
  ...
  if (!valResult.success) {
    console.error(
      ...
      JSON.stringify(valResult.error.errors, null, 2),
    );
    throw new Error("Validation failed");
  }
  console.log(
    "✅ Response payload validates against Zod contract schema successfully!",
  );
  ...
  console.log(
    "✅ Template retrieved and includes datasetConfigRelation successfully.",
  );
  ...
  console.log(
    "- Resolved Variables:",
    JSON.stringify(context.metadata.variables, null, 2),
  );
  console.log(
    "- Lineage Audit:",
    JSON.stringify(context.metadata.lineage, null, 2),
  );
  ...
  console.log(
    "✅ Prompt compiled and LLM stub mock response received successfully.",
  );
  ...
  console.log(
    "- Assembled Metadata (showing lineage):",
    JSON.stringify(assembled.metadata, null, 2),
  );
