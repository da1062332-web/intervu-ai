...
async function run() {
  ...
  if (configs.length === 0) {
    console.log("⚠️ No custom template prompt configurations found in the database. Let's create one for testing!");
    ...
    const created = await prisma.templatePromptConfig.upsert({
      where: { templateId },
      update: {
        systemPrompt: "CUSTOM SYSTEM PROMPT: You are a lexical assistant specialized in synonym validation.",
        userPrompt: "CUSTOM USER PROMPT: Validate the synonym {{synonym_placeholder}}.",
        instructions: "CUSTOM INSTRUCTIONS: Ensure the output matches the required JSON structure.",
      },
      create: {
        templateId,
        systemPrompt: "CUSTOM SYSTEM PROMPT: You are a lexical assistant specialized in synonym validation.",
    ...
    configs.push({
      ...created,
      template: (await prisma.template.findUnique({ where: { id: templateId } })) as any,
    });
  }
  ...
  const compiledPromptWithoutConfig = promptBuilder.buildPrompt(mockInputWithoutConfig as any);
  console.log("❌ Compiled Prompt (WITHOUT passing promptConfig - Current Buggy Controller):");
  console.log("--------------------------------------------------------------------------------");
  ...
  const compiledPromptWithConfig = promptBuilder.buildPrompt(mockInputWithConfig as any);
