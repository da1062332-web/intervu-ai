...
function coreLogic({ schemaPath }: { schemaPath: string }) {
  ...
  try {
    const output = execSync(`npx prisma migrate status --schema=${schemaPath}`, { encoding: 'utf-8' });
...
function formatResponse(result: any) {
  ...
  return {
    success,
    data: result,
    error: success ? null : { code: 'MIGRATION_AUDIT_FAILED', message: 'Migration drift or pending migrations detected.' },
