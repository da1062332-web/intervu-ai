...
async function run() {
  ...
  try {
    ...
    let user = await prisma.user.findFirst({
      where: { email: "tx-test-user@example.com" },
    });
    if (!user) {
    ...
    try {
      ...
      results.submissionRollback =
        sub && sub.status === SubmissionStatus.PENDING
          ? "PASS"
          : "FAIL_NOT_ROLLED_BACK";
    } finally {
      await prisma.testInstance
        .delete({ where: { id: instanceId3 } })
        .catch(() => {});
    }
    ...
    return {
      ...
      error: allPassed
        ? null
        : {
            code: "TRANSACTION_ROLLBACK_FAILURE",
            message: "One or more transaction rollback scenarios failed.",
          },
