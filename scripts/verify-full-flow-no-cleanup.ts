...
async function run() {
  ...
  try {
    ...
    const authHeader = {
      ...
      Authorization: `Bearer ${require("jsonwebtoken").sign(
        ...
        process.env.JWT_SECRET ||
          "replace-with-a-long-secret-at-least-32-chars",
      )}`,
    };
    ...
    if (!templateRes.ok || !templateData.success) {
      throw new Error(
        `Failed to create template: ${JSON.stringify(templateData)}`,
      );
    }
    ...
    const questionTextRes = await fetch(
      `${API_URL}/templates/${templateId}/question`,
      {
        method: "POST",
        headers: authHeader,
        body: JSON.stringify({
    ...
    if (!questionTextRes.ok || !questionTextData.success) {
      throw new Error(
        `Failed to set question text: ${JSON.stringify(questionTextData)}`,
      );
    }
    ...
    const optionsRes = await fetch(
      `${API_URL}/templates/${templateId}/options`,
      {
        method: "POST",
        headers: authHeader,
        body: JSON.stringify({
    ...
    console.log(
      "Step 6: API - Setting Difficulty Distribution (100% Medium)...",
    );
    ...
    if (!weightRes.ok || !weightData.success) {
      throw new Error(
        `Failed to assign weightage: ${JSON.stringify(weightData)}`,
      );
    }
    ...
    if (!workflowRes.ok || !workflowData.success) {
      throw new Error(
        `Failed to initialize workflow: ${JSON.stringify(workflowData)}`,
      );
    }
    ...
    if (!genRes.ok || !genData.success) {
      throw new Error(
        `Failed to generate questions: ${JSON.stringify(genData)}`,
      );
    }
    ...
    if (!fetchQuestionsRes.ok || !fetchQuestionsData.success) {
      throw new Error(
        `Failed to fetch draft questions: ${JSON.stringify(fetchQuestionsData)}`,
      );
    }
    ...
    console.log(
      `Advanced successfully. Current Step: ${advData.data.currentStep}\n`,
    );
    ...
    console.log(
      "Step 17.5: Database - Verifying active questions in candidate pool...",
    );
    ...
    console.log("==================================================\n");
    console.log(
      "⚠️ No cleanup has been run. You can inspect all data directly in the database.",
    );
  } catch (err) {
