    ...
    for (let a = 1; a <= totalAssessments; a++) {
      ...
      for (let q = 1; q <= 2; q++) {
        ...
        try {
          ...
          console.error(
            `Assessment #${a} Q#${q} Generation Error:`,
            error.message,
          );
          otherFailures++;
        }
      ...
      if (
        q1.questionText.trim().toLowerCase() ===
          q2.questionText.trim().toLowerCase() ||
        q1.questionId === q2.questionId
      ) {
        console.error(
          `❌ Assessment #${a} Fail: Duplicate questions in same assessment.`,
        );
        duplicateFailures++;
      }
      ...
      for (let idx = 0; idx < questionsInAssessment.length; idx++) {
        ...
        if (
          ...
          console.error(
            `❌ Assessment #${a} Q#${idx + 1} Fail: Missing correct answer.`,
          );
        ...
        if (
          ...
          console.error(
            `❌ Assessment #${a} Q#${idx + 1} Fail: Correct answer not in MCQ options list.`,
          );
          missingAnswerFailures++;
        }
        ...
        if (hasUnresolvedBraces || isMetadataEmpty) {
          console.error(
            `❌ Assessment #${a} Q#${idx + 1} Fail: Invalid metadata / unresolved placeholders.`,
          );
        ...
        if (!isDifficultyValid) {
          console.error(
            `❌ Assessment #${a} Q#${idx + 1} Fail: Invalid step count (${stepCount}) for difficulty '${difficulty}'.`,
          );
