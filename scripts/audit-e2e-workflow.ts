...
async function run() {
  ...
  try {
    ...
    console.error(
      "Please run the backend (npm run dev) first before running this script.",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    const topic = await prisma.topic.create({
      data: {
        ...
        description:
          "Topic created for testing the e2e exam generation audit flow",
        status: "ACTIVE",
      },
    });
    topicId = topic.id;
    ...
    const template = await prisma.template.create({
      data: {
        ...
        variableSchema: {
          ...
          formulas: ["C = price + tax", "opt1 = C + 10", "opt2 = C - 10"],
        },
        solutionSchema: {
          correctVariable: "C",
          explanationTemplate:
    ...
    console.log(
      `Topic ID: ${topicId}, Concept Code: ${concept.code}, Template ID: ${templateId}\n`,
    );
    ...
    const diffRes = await fetch(
      `${API_URL}/admin/configs/${examConfigId}/difficulty`,
      {
        method: "POST",
        headers: authHeader,
        body: JSON.stringify({
    ...
    if (!diffRes.ok || !diffData.success) {
      throw new Error(
        `Failed to set difficulty distribution: ${JSON.stringify(diffData)}`,
      );
    }
    ...
    const sectionRes = await fetch(
      `${API_URL}/admin/configs/${examConfigId}/sections`,
      {
        method: "POST",
        headers: authHeader,
        body: JSON.stringify({
    ...
    if (!sectionRes.ok || !sectionData.success) {
      throw new Error(
        `Failed to create section: ${JSON.stringify(sectionData)}`,
      );
    }
    ...
    const mapTopicRes = await fetch(
      `${API_URL}/admin/sections/${sectionId}/topics`,
      {
        method: "POST",
        headers: authHeader,
        body: JSON.stringify({
    ...
    const weightageRes = await fetch(
      `${API_URL}/admin/sections/${sectionId}/weightages`,
      {
        method: "POST",
        headers: authHeader,
        body: JSON.stringify({
    ...
    if (!weightageRes.ok || !weightageData.success) {
      throw new Error(
        `Failed to assign weightage: ${JSON.stringify(weightageData)}`,
      );
    }
    ...
    if (!startWorkflowRes.ok || !workflowData.success) {
      throw new Error(
        `Failed to start workflow: ${JSON.stringify(workflowData)}`,
      );
    }
    workflowId = workflowData.data.id;
    console.log(
      `Workflow started. Status: ${workflowData.data.status}, Step: ${workflowData.data.currentStep}\n`,
    );

    // 10. Call POST /workflows/:examId/generate (Generate Questions)
    console.log("6. API: Triggering Question Generation...");
    const genRes = await fetch(
      `${API_URL}/workflows/${examConfigId}/generate`,
      {
        method: "POST",
        headers: authHeader,
      },
    ...
    if (!genRes.ok || !genData.success) {
      throw new Error(
        `Failed to trigger generation: ${JSON.stringify(genData)}`,
      );
    }
    ...
    const getQsRes = await fetch(
      `${API_URL}/workflows/${examConfigId}/questions?page=1&limit=100`,
      {
        method: "GET",
        headers: authHeader,
      },
    ...
    if (!getQsRes.ok || !getQsData.success) {
      throw new Error(
        `Failed to fetch generated questions: ${JSON.stringify(getQsData)}`,
      );
    }
    ...
    if (questions.length === 0) {
      ...
      for (const log of logs) {
        console.log(
          `[${log.validationStage}] isValid: ${log.isValid} | reason: ${log.failureReason}`,
        );
      ...
      for (const log of genLogs) {
        console.log(
          `[${log.step}] status: ${log.status} | message: ${log.message}`,
        );
      }
    ...
    const approveRes = await fetch(
      `${API_URL}/workflows/${examConfigId}/questions/bulk-approve`,
      {
        method: "POST",
        headers: authHeader,
        body: JSON.stringify({
    ...
    if (!approveRes.ok || !approveData.success) {
      throw new Error(
        `Failed to bulk approve questions: ${JSON.stringify(approveData)}`,
      );
    }
    ...
    const adv1Res = await fetch(
      `${API_URL}/workflows/${examConfigId}/advance`,
      {
        method: "PATCH",
        headers: authHeader,
      },
    ...
    if (!adv1Res.ok || !adv1Data.success) {
      throw new Error(
        `Failed to advance workflow: ${JSON.stringify(adv1Data)}`,
      );
    }
    console.log(
      `Advanced successfully. Current Step: ${adv1Data.data.currentStep}\n`,
    );
    ...
    const assembleRes = await fetch(
      `${API_URL}/workflows/${examConfigId}/assemble`,
      {
        method: "POST",
        headers: authHeader,
      },
    ...
    if (!assembleRes.ok || !assembleData.success) {
      throw new Error(
        `Failed to run assembly: ${JSON.stringify(assembleData)}`,
      );
    }
    ...
    if (!assembledTest) {
      throw new Error(
        `AssembledTest record not found for configId: ${examConfigId}`,
      );
    }
    ...
    const versionRes = await fetch(
      `${API_URL}/assembly/${assembledTestId}/version`,
      {
        method: "POST",
        headers: authHeader,
      },
    ...
    if (!versionRes.ok || !versionData.success) {
      throw new Error(
        `Failed to create version snapshot: ${JSON.stringify(versionData)}`,
      );
    }
    ...
    if (!adv2Res.ok || !adv2Data.success) {
      throw new Error(
        `Failed to advance workflow: ${JSON.stringify(adv2Data)}`,
      );
    }
    console.log(
      `Advanced successfully. Current Step: ${adv2Data.data.currentStep}\n`,
    );

    // 16. Call POST /workflows/:examId/publish (Publish Exam)
    console.log("12. API: Publishing Exam...");
    const publishRes = await fetch(
      `${API_URL}/workflows/${examConfigId}/publish`,
      {
        method: "POST",
        headers: authHeader,
      },
    ...
    if (!publishRes.ok || !publishData.success) {
      throw new Error(
        `Failed to publish workflow: ${JSON.stringify(publishData)}`,
      );
    }
    ...
    console.log(
      `Active questions found in candidate pool: ${activeQuestions.length}`,
    );
    for (const q of activeQuestions) {
      console.log(
        `- Question Text: "${q.questionText}" | Correct Answer: "${q.answer}"`,
      );
    }

    if (activeQuestions.length === 0) {
      throw new Error(
        "Verification failed: Questions not copied to the active candidate pool.",
      );
    }
    ...
    if (examConfigId) {
      await prisma.assemblyVersion.deleteMany({
        where: { assemblyId: examConfigId },
      });
      await prisma.assembledTest.deleteMany({
        where: { configId: examConfigId },
      });
