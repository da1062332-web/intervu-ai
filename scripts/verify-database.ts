  ...
  for (const file of list) {
    ...
    if (stat.isDirectory()) {
      ...
      const isRepository =
        ...
        filePath.includes(`${path.sep}repositories${path.sep}`);
      const isQueueWorker =
        file.endsWith(".queue.ts") ||
        filePath.includes(`${path.sep}queues${path.sep}`);
      ...
      const directQueryPattern =
        /\bprisma\.(user|session|refreshToken|template|test|testConfig|testSection|testRule|generatedQuestion|testInstance|testInstanceSection|testInstanceQuestion|candidateAnswer|executionState|submission|recommendation|performanceSummary)\b/i;
      const thisPrismaQueryPattern =
        /this\.prisma\.(user|session|refreshToken|template|test|testConfig|testSection|testRule|generatedQuestion|testInstance|testInstanceSection|testInstanceQuestion|candidateAnswer|executionState|submission|recommendation|performanceSummary)\b/i;

      if (
        directQueryPattern.test(content) ||
        thisPrismaQueryPattern.test(content)
      ) {
  ...
  try {
    ...
    const queries = [
      {
        ...
        fn: () =>
          prisma.user.findUnique({
            where: { id: "non-existent-user-id-to-test-index" },
          }),
      },
      {
        ...
        fn: () =>
          prisma.testInstance.findMany({
            where: { userId: "non-existent-user-id-to-test-index" },
            take: 10,
          }),
      },
      {
      ...
      },
      {
        ...
        fn: () =>
          prisma.testInstanceQuestion.findMany({
            where: { questionId: "non-existent-q-id-to-test-index" },
            take: 10,
          }),
      },
    ];
    ...
    return {
      ...
      error: allPassed
        ? null
        : {
            code: "DATABASE_VERIFICATION_FAILURE",
            message: "Database audit or performance latency checks failed.",
          },
