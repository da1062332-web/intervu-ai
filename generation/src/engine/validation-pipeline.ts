  ...
  for (const variable of variables) {
    ...
    if (variable.type === "number") {
      if (typeof val !== "number") {
        issues.push(
          `Variable ${variable.name} must be a number, got ${typeof val}.`,
        );
        continue;
      }
      ...
      if (val < min || val > max) {
        issues.push(
          `Variable ${variable.name} value ${val} is out of range [${min}, ${max}].`,
        );
      }
    } else if (variable.type === "string") {
      if (typeof val !== "string") {
        issues.push(
          `Variable ${variable.name} must be a string, got ${typeof val}.`,
        );
        continue;
      }
      if (!variable.options.includes(val)) {
        issues.push(
          `Variable ${variable.name} value "${val}" is not in options [${variable.options.join(", ")}].`,
        );
      }
    }
  }
 ...
 */
export function validateConstraintsPipeline(
...
export function validateSolvability(
  ...
  parameters: Record<string, unknown>,
): { isSolvable: boolean; answer: unknown; error?: string } {
  try {
    ...
    if (
      ...
      return {
        isSolvable: false,
        answer: null,
        error: `Evaluated answer is not a finite number: ${answer}`,
      };
    }
...
export function validateAmbiguity(
  ...
  if (distractors.includes(correctAnswer)) {
    issues.push(
      `Ambiguity detected: Correct answer "${correctAnswer}" is also one of the distractors.`,
    );
  }
...
export function validateDuplicate(
  ...
  if (hashesSet.has(hash)) {
    issues.push(
      `Duplicate question detected: Hash "${hash}" has already been generated.`,
    );
  }
  return issues;
}
...
export function validatePipeline(options: {
  ...
  criticalIssues.push(
    ...validateVariables(options.template.variables, options.parameters),
  );

  // 2. Constraint Validation
  const evalResult = evaluateConstraints(
    options.template.constraints,
    options.parameters,
  );
  for (const c of evalResult.violatedConstraints) {
    if (c.severity === "critical") {
  ...
  const solvability = validateSolvability(
    options.template.solutionTemplate.finalAnswer,
    options.parameters,
  );
  if (!solvability.isSolvable) {
    ...
    if (ans.trim() === "" || (isNaN(Number(ans)) && ans === "NaN")) {
      criticalIssues.push(
        `Answer correctness check failed: evaluated value is invalid: ${ans}`,
      );
    }
  }
  ...
  criticalIssues.push(
    ...validateAmbiguity(options.correctAnswer, options.distractors),
  );

  // 5. Output Schema Validation
  if (options.distractors.length !== 3) {
    criticalIssues.push(
      `Schema validation failed: expected 3 distractors, got ${options.distractors.length}`,
    );
  }
  ...
  if (
    options.pastParameters &&
    checkVariableCollision(options.parameters, options.pastParameters)
  ) {
    criticalIssues.push(
      `Variable collision detected: parameters match a previous generation run`,
    );
  }

  // 8. Difficulty Rules Validation
  const difficultyCheck = validateDifficulty(
    options.template,
    options.parameters,
  );
  ...
  if (options.hydratedQuestion && options.pastQuestionTexts) {
    for (const pastText of options.pastQuestionTexts) {
      if (isSemanticallySimilar(options.hydratedQuestion, pastText)) {
        criticalIssues.push(
          `Semantic duplicate detected: similarity threshold exceeded against past question`,
        );
        break;
      }
    }
