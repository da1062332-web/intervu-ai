...
2. **Session Deletion**: `useDeleteSession` and `useDeleteAllSessions` invalidate `userQueryKeys.sessions()`. 
   *Note*: `useDeleteSession` implements optimistic updates to immediately remove the session from the UI before the network request completes, rolling back if it fails.
