...
export default function AnalyticsDashboardPage() {
  ...
  const getIconForStat = (title: string) => {
    switch (title) {
      case 'Tests Taken': return <FileText className="size-5" />;
      case 'Average Score': return <TrendingUp className="size-5" />;
      case 'Pass Rate': return <CheckCircle2 className="size-5" />;
      case 'Active Candidates': return <Users className="size-5" />;
            ...
            {isLoading ? (
              ...
              mockPerformanceMetrics.map((metric, idx) => (
                <ProgressIndicator
                  ...
                  color={metric.score > 80 ? "bg-emerald-500" : metric.score > 60 ? "bg-blue-500" : "bg-amber-500"}
                />
              ))
            ) : (
              <EmptyAnalyticsState title="No skills tracked" description="Performance metrics will appear here." />
            )}
          </CardContent>
        </Card>
      </div>
          ...
          {isLoading ? (
            ...
            <div className="divide-y divide-border">
              {mockRecentActivity.map((activity) => (
                <div key={activity.id} className="p-6 flex items-start gap-4 hover:bg-muted/50 transition-colors">
                  <div className={`size-10 rounded-full flex flex-shrink-0 items-center justify-center mt-1 ${
                    activity.type === 'interview' ? 'bg-blue-500/10 text-blue-500' :
                    activity.type === 'assessment' ? 'bg-emerald-500/10 text-emerald-500' :
            ...
            <div className="p-6">
              <EmptyAnalyticsState title="No recent activity" description="Activity will appear here once candidates take interviews." />
            </div>
          )}
        </CardContent>
      </Card>
