...
export function ConstraintBuilderSection() {
  ...
  const handleSave = () => {
    ...
    const ruleString =
      formData.operator === 'Formula' || formData.operator === 'Custom'
        ? formData.value
    ...
    if (editingId) {
      newConstraintsArr = currentConstraints.map((c: any) =>
        c.rule === editingId ? newConstraint : c,
      );
    } else {
  ...
  return (
    ...
    >
      <div className='mb-4 rounded-lg border border-indigo-200 bg-indigo-50 px-4 py-3 text-sm text-indigo-900 dark:border-indigo-800 dark:bg-indigo-950/40 dark:text-indigo-100'>
        Note: For advanced generation control, preferred use the{' '}
        <strong>Generation Strategy</strong> tab instead of the legacy Constraint Builder. This
        sidebar is temporarily shown for review and can be removed in the next phase.
      </div>
