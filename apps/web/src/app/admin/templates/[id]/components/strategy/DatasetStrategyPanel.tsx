...
export function DatasetStrategyPanel({ templateId: _, template }: StrategyPanelProps) {
  ...
  const displayDatasets =
    ...
    (templateTopicIds.length === 0 && templateConceptKeys.length === 0)
      ? matchedDatasets.length > 0
        ? matchedDatasets
        : datasets || []
      : datasets || [];
      ...
      >
        <div className='p-3 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-md mb-4'>
          <p className='text-sm text-blue-800 dark:text-blue-300'>
            ...
            or vocabulary item from a dataset. Ideal for reading comprehension and contextual
            questions.
          </p>
        </div>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-3'>
              {displayDatasets.map((dataset: Dataset) => {
                const isMatchingTopic =
                  (templateTopicIds.length > 0 && dataset.topicId
                    ? templateTopicIds.includes(dataset.topicId)
                    : false) ||
                  (templateConceptKeys.length > 0 && dataset.conceptId
                    ? templateConceptKeys.includes(dataset.conceptId)
                    : false);
                return (
                  <div
                    key={dataset.id}
