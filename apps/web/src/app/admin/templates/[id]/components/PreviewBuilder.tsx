...
const Textarea = (props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) => (
  <textarea {...props} className={`flex min-h-[80px] w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 ${props.className || ''}`} />
);

export function PreviewBuilder() {
  const params = useParams();
  const templateId = params.id as string;
  const { previewInput, setPreviewInput, setPreviewResult } = useTemplatePreviewStore();
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (e instanceof SyntaxError) {
        ...
        setPreviewResult(e.response?.data?.data?.previewResult || {
