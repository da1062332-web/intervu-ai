...
export function BlueprintHealthWidget(props: BlueprintHealthWidgetProps) {
  ...
  const profileSelected =
    !!selectedStyleProfileId && profiles?.some((p) => p.id === selectedStyleProfileId);
  const sectionsConfigured = sections.length > 0;

  const topicAllocationValid =
    sections.length > 0 &&
    sections.every((s: BlueprintSectionPayload) => {
      const sum = s.topicAllocations.reduce((acc, t) => acc + (t.percentage || 0), 0);
      return sum === 100;
    });

  const difficultyAllocationValid =
    sections.length > 0 &&
    sections.every((s: BlueprintSectionPayload) => {
  ...
  const isReady =
    ...
    profileSelected &&
    sectionsConfigured &&
    topicAllocationValid &&
    difficultyAllocationValid &&
    templateTypesSelected;
    ...
    <div className='p-6 border rounded-lg bg-background shadow-sm space-y-4'>
      ...
      <ul className='space-y-3'>
        <li className='flex items-center gap-3'>
          ...
          <span className={configSelected ? 'text-foreground' : 'text-muted-foreground'}>
            Config Selected
          </span>
        </li>
        <li className='flex items-center gap-3'>
          ...
          <span className={profileSelected ? 'text-foreground' : 'text-muted-foreground'}>
            Style Profile Selected
          </span>
        </li>
        <li className='flex items-center gap-3'>
          {sectionsConfigured ? (
          ...
          <span className={sectionsConfigured ? 'text-foreground' : 'text-muted-foreground'}>
            Sections Configured
          </span>
        </li>
        <li className='flex items-center gap-3'>
          ...
          <span className={topicAllocationValid ? 'text-foreground' : 'text-muted-foreground'}>
            Topic Allocation (100%)
          </span>
        </li>
        <li className='flex items-center gap-3'>
          {difficultyAllocationValid ? (
          ...
          <span className={difficultyAllocationValid ? 'text-foreground' : 'text-muted-foreground'}>
            Difficulty Mix (100%)
          </span>
        </li>
        <li className='flex items-center gap-3'>
          ...
          <span className={templateTypesSelected ? 'text-foreground' : 'text-muted-foreground'}>
            Template Types Selected
          </span>
        </li>
      </ul>

      <div
        className={`p-3 rounded-md border mt-4 ${isReady ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}
      >
        <p
          className={`font-semibold flex items-center gap-2 ${isReady ? 'text-green-700' : 'text-red-700'}`}
        >
          {isReady ? (
            <>
