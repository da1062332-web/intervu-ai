      ...
      {!isLoading && !isError && previewData && (
        <div className='mt-4 space-y-6'>
          <div className='flex gap-4 text-sm text-gray-500 bg-gray-50 dark:bg-gray-800/50 p-3 rounded-md'>
            <div>
              <span className='font-semibold text-gray-700 dark:text-gray-300'>Config ID:</span>{' '}
              {previewData.configId}
            </div>
            <div>
              <span className='font-semibold text-gray-700 dark:text-gray-300'>Profile ID:</span>{' '}
              {previewData.styleProfileId}
            </div>
          </div>
                ...
                <div>
                  ...
                  <div className='space-y-2'>
                    {section.topics?.map((topic: any, tIdx: number) => (
                      <div
                        key={tIdx}
                        className='flex justify-between items-center text-sm border-b pb-1 border-dashed'
                      >
                ...
                <div>
                  ...
                  <div className='flex gap-2 flex-wrap'>
                    {Object.entries(section.difficultyAllocation || {}).map(([level, count]) => (
                      <div
                        key={level}
                        className='bg-gray-50 dark:bg-gray-800 border px-3 py-1.5 rounded-md text-sm flex items-center gap-2'
                      >
                  ...
                  <div className='mt-4'>
                    ...
                    <div className='flex gap-2 flex-wrap'>
                      {section.templateTypes?.map((type: string, typeIdx: number) => (
                        <span
                          key={typeIdx}
                          className='bg-indigo-100 text-indigo-800 dark:bg-indigo-900/50 dark:text-indigo-300 px-2 py-0.5 rounded text-xs'
                        >
                          {type}
                        </span>
                      ))}
