  ...
  return (
    <div className="space-y-4 p-4 border rounded-lg bg-gray-50/30 dark:bg-gray-900/30">
      <div className="mb-2">
        ...
        <p className="text-xs text-muted-foreground mt-1">Select whether the correct answer is True or False.</p>
      </div>

      <div className="flex items-center space-x-6">
        <label className={`flex items-center space-x-3 p-3 border rounded-lg cursor-pointer transition-colors ${
          answer === 'True' ? 'border-emerald-500 bg-emerald-50/20' : 'hover:bg-gray-50 dark:hover:bg-gray-800'
        }`}>
          <input
            type="radio"
            name={`tf-${index}`}
        ...
        <label className={`flex items-center space-x-3 p-3 border rounded-lg cursor-pointer transition-colors ${
          answer === 'False' ? 'border-emerald-500 bg-emerald-50/20' : 'hover:bg-gray-50 dark:hover:bg-gray-800'
        }`}>
          <input
            type="radio"
            name={`tf-${index}`}
