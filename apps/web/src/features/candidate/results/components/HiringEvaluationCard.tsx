      ...
      <CardHeader className='bg-muted/10 pb-4 border-b'>
        <div className='flex items-center justify-between'>
          <div className='space-y-1'>
            ...
            <CardDescription>
              Automated corporate hiring tier assessment & sectional eligibility analysis.
            </CardDescription>
          </div>
          <Badge
            className={`px-4 py-1.5 text-sm font-semibold border ${getBadgeStyle(qualification)}`}
          >
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
          ...
          <div className='p-4 border rounded-lg bg-card space-y-3'>
            <div className='flex items-center justify-between border-b pb-2'>
              ...
              <span className='text-lg font-bold'>
                {fBreakdown?.foundationTotal ?? foundationScore}
              </span>
            </div>
            ...
            <div className='space-y-1.5 text-xs'>
              <div className='flex justify-between text-muted-foreground'>
                ...
                <span className='font-medium text-foreground'>
                  {fBreakdown?.ninjaThreshold ?? 0}
                </span>
              </div>
              <div className='flex justify-between text-muted-foreground'>
                <span>Digital Threshold:</span>
                <span className='font-medium text-foreground'>
                  {fBreakdown?.digitalThreshold ?? 0}
                </span>
              </div>
              <div className='flex justify-between text-muted-foreground'>
                <span>Prime Threshold:</span>
                <span className='font-medium text-foreground'>
                  {fBreakdown?.primeThreshold ?? 0}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className='p-4 border rounded-lg bg-card space-y-3'>
            <div className='flex items-center justify-between border-b pb-2'>
              ...
              <span className='text-lg font-bold'>
                {aBreakdown?.advancedScore ?? advancedScore}
              </span>
            </div>
            ...
            <div className='space-y-1.5 text-xs'>
              <div className='flex justify-between text-muted-foreground'>
                ...
                <span className='font-medium text-foreground'>
                  {aBreakdown?.advancedMinDigital ?? 0}
                </span>
              </div>
              <div className='flex justify-between text-muted-foreground'>
                <span>Prime Min Required:</span>
                <span className='font-medium text-foreground'>
                  {aBreakdown?.advancedMinPrime ?? 0}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className='p-4 border rounded-lg bg-card space-y-3'>
            ...
            <div className='space-y-1.5 text-xs'>
              <div className='flex justify-between text-muted-foreground'>
                ...
                <span className='font-medium text-foreground'>
                  {cBreakdown?.codingMinDigital ?? 0}
                </span>
              </div>
              <div className='flex justify-between text-muted-foreground'>
                <span>Prime Min Solved:</span>
                <span className='font-medium text-foreground'>
                  {cBreakdown?.codingMinPrime ?? 0}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className='space-y-2'>
            ...
            <div className='border rounded-lg overflow-hidden divide-y text-xs'>
              {fBreakdown.sectionsBreakdown.map((sec, idx) => (
                <div
                  key={idx}
                  className='flex items-center justify-between p-3 bg-muted/10 hover:bg-muted/20 transition-colors'
                >
                  <div className='flex items-center gap-2'>
                    {sec.passed ? (
                    ...
                    <span className='font-medium'>
                      {sec.sectionName || sec.sectionCode} ({sec.category})
                    </span>
                  </div>
                  <div className='flex items-center gap-4 text-muted-foreground'>
                    <span>
                      Correct: <strong className='text-foreground'>{sec.correctCount}</strong> / Req
                      Min: <strong className='text-foreground'>{sec.requiredMin}</strong>
                    </span>
                    <Badge
                      variant={sec.passed ? 'outline' : 'destructive'}
