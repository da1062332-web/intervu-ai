...
export function ConceptManualQuestionsModal({ isOpen, onClose, concept }: ConceptManualQuestionsModalProps) {
  const { data: response, isLoading, isError } = useManualQuestions(
    ...
    <Modal isOpen={isOpen} onClose={onClose} className="max-w-3xl max-h-[80vh] flex flex-col">
      <div className="flex-none pb-4 border-b">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Viewing questions for concept: <span className="font-medium text-foreground">{concept.name || concept.conceptName}</span>
        </p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-4">
        {isLoading ? (
          ...
          <div className="text-center py-8 text-destructive">
            Failed to load manual questions.
          </div>
        ) : questions.length === 0 ? (
          <div className="text-center py-12 border-2 border-dashed rounded-lg bg-muted/10">
            <p className="text-muted-foreground mb-4">No manual questions found for this concept.</p>
          </div>
        ) : (
