...
export function TopicMappingHealthWidget({ topics }: TopicMappingHealthWidgetProps) {
  ...
  return (
    <div className='p-6 border rounded-lg bg-background shadow-sm space-y-4'>
      ...
      <div className={`p-3 rounded-md border mt-4 ${isReady ? 'bg-green-50 border-green-200 dark:bg-green-900/10 dark:border-green-800' : 'bg-red-50 border-red-200 dark:bg-red-900/10 dark:border-red-800'}`}>
        <p className={`font-semibold flex items-center gap-2 ${isReady ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400'}`}>
          {isReady ? (
            <>
