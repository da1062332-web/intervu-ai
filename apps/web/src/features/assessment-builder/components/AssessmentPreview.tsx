  ...
  return (
    <div className="flex flex-col md:flex-row gap-6">
      ...
      <div className="w-full md:w-64 flex-shrink-0 space-y-2">
        <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider mb-4 px-2">Sections</h3>
        {sections.map(section => (
          ...
          >
            <div className="flex justify-between items-center">
              ...
              <Badge variant={activeSectionId === section.id ? 'secondary' : 'outline'} className="text-xs">
        ...
        {questions.length === 0 ? (
          ...
          <div className="space-y-4">
            {currentQuestions.map((q, idx) => (
              <QuestionCard key={q.id} question={q} index={(currentPage - 1) * QUESTIONS_PER_PAGE + idx + 1} />
            ))}
    ...
    <Card>
      <CardHeader className="pb-3 flex flex-row items-start justify-between space-y-0">
        ...
        <div className="flex gap-2 items-center flex-wrap">
          <Badge variant={question.difficulty === 'HARD' ? 'destructive' : question.difficulty === 'EASY' ? 'secondary' : 'default'} className="flex-shrink-0">
            {question.difficulty}
          </Badge>
          {question.topicId && (
            <Badge variant="outline" className="flex-shrink-0 text-muted-foreground border-muted-foreground/30">
              {question.topicId}
            </Badge>
          )}
          {question.conceptKey && (
        ...
        {question.options && question.options.length > 0 && (
          <div className="pl-11 space-y-2 mt-2">
            {question.options.map((opt, i) => (
              ...
              >
                {opt}
                {opt === question.answer && <span className="ml-2 text-green-600 text-xs uppercase font-bold tracking-wider">(Correct Answer)</span>}
              </div>
            ))}
          </div>
        )}
