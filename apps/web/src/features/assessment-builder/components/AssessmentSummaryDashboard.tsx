      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-4 flex flex-col justify-center">
            ...
            <span className="font-mono text-xs font-bold truncate" title={assessment.testId}>{assessment.testId}</span>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 flex flex-col justify-center">
            <span className="text-sm text-muted-foreground mb-1">Status</span>
            <Badge variant="default" className="w-fit">{assessment.status}</Badge>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 flex flex-col justify-center">
              ...
              {validation.suggestions.length > 0 && (
                <div>
                  ...
                  <ul className="list-disc pl-5 text-sm text-blue-700 space-y-1">
                    {validation.suggestions.map((sug, i) => <li key={i}>{sug}</li>)}
                  </ul>
                </div>
              )}
            </div>
