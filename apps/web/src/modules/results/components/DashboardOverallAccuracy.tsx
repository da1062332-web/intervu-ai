...
export const DashboardOverallAccuracy: React.FC<Props> = ({ data }) => {
  ...
  const getSectionIcon = (name: string) => {
    ...
    if (lower.includes('numerical') || lower.includes('quant') || lower.includes('math')) {
      return <div className="p-1.5 rounded-md bg-indigo-500/10 text-indigo-600 dark:text-indigo-400"><Calculator className="w-3.5 h-3.5" /></div>;
    }
    if (lower.includes('reasoning') || lower.includes('logic')) {
      return <div className="p-1.5 rounded-md bg-emerald-500/10 text-emerald-600 dark:text-emerald-400"><Brain className="w-3.5 h-3.5" /></div>;
    }
    if (lower.includes('verbal') || lower.includes('english')) {
      return <div className="p-1.5 rounded-md bg-blue-500/10 text-blue-600 dark:text-blue-400"><BookOpen className="w-3.5 h-3.5" /></div>;
    }
    return <div className="p-1.5 rounded-md bg-purple-500/10 text-purple-600 dark:text-purple-400"><Code2 className="w-3.5 h-3.5" /></div>;
  };
