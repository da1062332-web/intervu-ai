  ...
  const cleanRemarks = (str: string) => {
    ...
    return str
      .replace(
        ...
        'Topic requires foundational concept practice.',
      )
          ...
          {data.strengths.length === 0 ? (
            ...
            <ul className='space-y-4'>
              {data.strengths.map((item, i) => (
                <li
                  key={i}
                  className='flex flex-col border-b border-gray-200 dark:border-slate-800 last:border-0 pb-3 last:pb-0'
                >
                  <div className='flex justify-between items-center mb-1'>
                    <span
                      className='font-semibold text-gray-800 dark:text-gray-200 truncate max-w-[80%]'
                      title={item.topic}
                    >
                      {item.topic}
                    </span>
