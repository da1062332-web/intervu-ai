...
export const DashboardPerformanceSummary: React.FC<Props> = ({ data }) => {
  ...
  const completionRate = totalQuestions > 0 ? Math.round((attemptedQuestions / totalQuestions) * 100) : 100;
  
  const totalSecs = (data.totalTimeSpent || 0) * 60;
  const avgSecsPerQ = totalQuestions > 0 ? Math.round(totalSecs / totalQuestions) : 0;
  const formattedAvgSpeed = avgSecsPerQ >= 60 ? `${(avgSecsPerQ / 60).toFixed(1)}m` : `${avgSecsPerQ}s`;

  const ratio = wrong > 0 ? (correct / wrong).toFixed(1) : correct > 0 ? `${correct}:0` : '0';

  let executiveInsight = "Good overall attempt rate. Review flagged questions to boost overall accuracy.";
  if (completionRate >= 90 && data.overallAccuracy >= 70) {
    executiveInsight = "Outstanding performance! High completion rate combined with strong accuracy.";
  } else if (completionRate >= 90 && data.overallAccuracy < 50) {
    executiveInsight = "100% attempt rate with fast pacing. Focus on double-checking answers before submitting to reduce errors.";
  } else if (data.overallAccuracy >= 70) {
    executiveInsight = "Strong accuracy on attempted questions. Work on speed to complete more questions in future tests.";
  }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div className="p-3 bg-slate-800/80 rounded-xl border border-slate-700/80 space-y-1.5">
            ...
            <div className="text-lg font-bold text-white">
              {attemptedQuestions}/{totalQuestions} <span className="text-xs font-normal text-slate-400">Attempted</span>
            </div>
          ...
          <div className="p-3 bg-slate-800/80 rounded-xl border border-slate-700/80 space-y-1.5">
            ...
            <div className="text-lg font-bold text-white">
              {formattedAvgSpeed} <span className="text-xs font-normal text-slate-400">/ question</span>
            </div>
            <div className="text-[11px] text-slate-400">
              Total {data.totalTimeSpent} mins spent
            </div>
          </div>
        </div>
        ...
        <div className="p-3 bg-slate-800/80 rounded-xl border border-slate-700/80 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-xs text-slate-500">{correct} correct vs {wrong} wrong</div>
            </div>
          </div>
