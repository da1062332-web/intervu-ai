...
export function AssessmentStatusPanel() {
      ...
      <Card className='bg-card/80 border border-border/60 shadow-xs h-full flex flex-col'>
        <CardHeader className='pb-3 border-b border-border/40'>
          ...
          <CardDescription className='text-xs text-muted-foreground font-medium'>
            Active evaluations and pending test assignments
          </CardDescription>
        </CardHeader>
  ...
  const hasContent =
    inProgressTests.length > 0 || enrolledTests.length > 0 || reattemptableTests.length > 0;
          ...
          .map((test, idx) => (
            ...
            >
              ...
              <Button
                ...
                size='sm'
                variant='outline'
                className='gap-1.5 font-semibold text-xs h-8 shrink-0 border-border/60 hover:border-primary'
              >
