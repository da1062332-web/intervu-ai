        ...
        <div className='rounded-md border overflow-x-auto'>
          <table className='w-full text-sm text-left'>
            <thead className='bg-muted/50 text-muted-foreground border-b'>
              <tr>
                <th className='px-4 py-3 font-medium cursor-pointer hover:text-foreground' onClick={() => toggleSort('assessmentName')} aria-sort={sortField === 'assessmentName' ? (sortAsc ? 'ascending' : 'descending') : 'none'}>
                  <div className='flex items-center gap-1'>Assessment <ArrowUpDown className='size-3 opacity-50' /></div>
                </th>
                <th className='px-4 py-3 font-medium cursor-pointer hover:text-foreground' onClick={() => toggleSort('date')} aria-sort={sortField === 'date' ? (sortAsc ? 'ascending' : 'descending') : 'none'}>
                  <div className='flex items-center gap-1'>Date <ArrowUpDown className='size-3 opacity-50' /></div>
                </th>
                <th className='px-4 py-3 font-medium cursor-pointer hover:text-foreground' onClick={() => toggleSort('status')} aria-sort={sortField === 'status' ? (sortAsc ? 'ascending' : 'descending') : 'none'}>
                  <div className='flex items-center gap-1'>Status <ArrowUpDown className='size-3 opacity-50' /></div>
                </th>
                <th className='px-4 py-3 font-medium text-right cursor-pointer hover:text-foreground' onClick={() => toggleSort('score')} aria-sort={sortField === 'score' ? (sortAsc ? 'ascending' : 'descending') : 'none'}>
