...
const getStepStatusObj = (details: WorkflowStatusDetails, step: WorkflowStep): StepStatus => {
  switch (step) {
    ...
    case WorkflowStep.QUESTION_GENERATION:
      return details.questionGeneration;
    case WorkflowStep.QUESTION_REVIEW:
      return details.questionReview;
    case WorkflowStep.ASSEMBLY:
      return details.assembly;
    case WorkflowStep.PUBLISHING:
      return details.publishing;
    default:
      return { status: WorkflowStatus.NOT_STARTED, progress: 0 };
  }
};
          ...
          <div key={step} className='relative pl-8'>
            <div className={cn('absolute -left-[11px] top-1 p-0.5 rounded-full', iconClass)}>
              <Icon
                className={cn(
                  'h-5 w-5',
                  statusObj.status === WorkflowStatus.IN_PROGRESS && 'animate-spin',
                )}
              />
            </div>
