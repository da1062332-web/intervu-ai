    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Make the final test available for candidates by publishing it to the selected environment.
          </p>
        </div>
        <div className="flex gap-3">
          ...
          <Button onClick={onPublish} disabled={status.status === 'IN_PROGRESS' || status.status === 'COMPLETED'}>
            <Send className="w-4 h-4 mr-2" />
            {status.status === 'IN_PROGRESS' ? 'Publishing...' : status.status === 'COMPLETED' ? 'Published' : 'Publish Test'}
          </Button>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <div className="rounded-lg border bg-card p-6 shadow-sm flex flex-col justify-center items-center text-center">
          <div className={`p-4 rounded-full mb-3 ${status.status === 'COMPLETED' ? 'bg-emerald-100 text-emerald-600' : 'bg-primary/10 text-primary'}`}>
            <Globe className="w-8 h-8" />
          </div>
          <div className="text-sm font-medium text-muted-foreground">Test Status</div>
          <div className={`text-xl font-bold capitalize mt-1 ${status.status === 'COMPLETED' ? 'text-emerald-600' : 'text-primary'}`}>
        ...
        <div className="md:col-span-2 rounded-lg border bg-card p-6 shadow-sm">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between text-sm items-center">
              ...
              <span className="font-medium">{status.finishedAt ? new Date(status.finishedAt).toLocaleString() : 'Not published yet'}</span>
            </div>
            <div className="flex justify-between text-sm items-center">
      ...
      {status.status === 'COMPLETED' && (
        <div className="flex justify-end">
          <Button variant="ghost" className="text-red-600 hover:text-red-700 hover:bg-red-50" onClick={handleRollback}>
