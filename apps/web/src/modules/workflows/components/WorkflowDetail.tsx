...
export const WorkflowDetail: React.FC<WorkflowDetailProps> = ({ examId }) => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'generation' | 'review' | 'assembly' | 'publishing' | 'history'>('overview');
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="bg-card rounded-lg border shadow-sm p-6 min-h-[500px]">
            ...
            {activeTab === 'generation' && <GenerationPanel status={details.questionGeneration} onGenerate={async () => {
              try { await startGeneration(); } catch (e: any) { alert(e.message); }
            }} />}
            {activeTab === 'review' && <ReviewPanel examId={examId} status={details.questionReview} onReview={() => advanceWorkflow()} />}
            {activeTab === 'assembly' && <AssemblyPanel status={details.assembly} onAssemble={async () => {
              try { await startAssembly(); } catch (e: any) { alert(e.message); }
            }} />}
            {activeTab === 'publishing' && <PublishingPanel status={details.publishing} onPublish={async () => {
              try { await publishWorkflow(); } catch (e: any) { alert(e.message); }
            }} />}
