    ...
    <div className="grid gap-6 md:grid-cols-4 mb-8">
      {cards.map((card, idx) => {
        ...
        return (
          ...
          >
            <div className={`absolute inset-0 bg-gradient-to-br opacity-50 transition-opacity duration-300 group-hover:opacity-100 ${card.gradient}`} />
            
            <div className="relative p-6">
              <div className="flex flex-row items-center justify-between space-y-0 pb-2">
                <h3 className="tracking-tight text-sm font-medium text-muted-foreground">{card.title}</h3>
                <div className={`p-2 rounded-full bg-background/50 backdrop-blur-sm shadow-sm ring-1 ring-black/5 ${card.iconColor}`}>
                  <Icon className="h-4 w-4" />
                </div>
              </div>
              <div className="flex flex-col mt-2">
                <span className="text-3xl font-bold tracking-tight">{card.value}</span>
                <span className="text-xs text-muted-foreground mt-1 font-medium">{card.subtitle}</span>
              </div>
            </div>
          </div>
        );
