          ...
          <div className='space-y-3'>
            <div className='flex justify-between items-center'>
              ...
              <span
                className={`text-xs font-semibold px-2 py-1 rounded-full ${
                  overview.workflowStatus === 'COMPLETED'
                    ? 'bg-emerald-100 text-emerald-700'
                    : overview.workflowStatus === 'IN_PROGRESS'
                      ? 'bg-violet-100 text-violet-700'
                      : overview.workflowStatus === 'FAILED'
                        ? 'bg-red-100 text-red-700'
                        : 'bg-slate-100 text-slate-600'
                }`}
              >
                {STATUS_LABELS[overview.workflowStatus] ?? overview.workflowStatus}
              </span>
            </div>
            <div className='flex justify-between items-center'>
              <span className='text-sm text-muted-foreground'>Current Step</span>
              <span className='text-sm font-medium'>
                {overview.currentStep?.replace(/_/g, ' ')}
              </span>
            </div>
            ...
            {overview.publishedAt && (
              <div className='flex justify-between items-center'>
                ...
                <span className='text-sm font-medium'>
                  {new Date(overview.publishedAt).toLocaleDateString()}
                </span>
              </div>
            )}
          </div>
      ...
      <div>
        ...
        <div className='grid grid-cols-2 sm:grid-cols-4 gap-3'>
          {[
            ...
            {
              label: 'Pending',
              value: questionStats.draft,
              color: 'border-amber-200 bg-amber-50 text-amber-700',
            },
            {
              label: 'Approved',
              value: questionStats.approved,
              color: 'border-emerald-200 bg-emerald-50 text-emerald-700',
            },
            {
              label: 'Rejected',
              value: questionStats.rejected,
        ...
        <div>
          ...
          <div className='rounded-xl border overflow-hidden divide-y'>
            {overview.sections.map((section: any, i: number) => (
              <div
                key={section.id}
                className='px-4 py-3 flex items-center justify-between gap-4 bg-card hover:bg-muted/20 transition-colors'
              >
                <div>
                  <p className='text-sm font-medium'>{section.name}</p>
                  {section.topics?.length > 0 && (
                    <p className='text-xs text-muted-foreground mt-0.5'>
                      Topics: {section.topics.slice(0, 4).join(', ')}
