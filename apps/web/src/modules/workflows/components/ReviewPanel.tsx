...
import {
  CheckSquare, XCircle, Search, Filter, Loader2,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Approved', value: approvedCount, color: 'bg-emerald-50 border-emerald-200 text-emerald-700' },
            ...
            <div key={q.id} className="bg-card hover:bg-muted/20 transition-colors">
              ...
              <div className="px-4 py-3 flex items-start gap-3">
                ...
                <div className={`mt-1 flex-shrink-0 w-2 h-2 rounded-full ${
                  ...
                  q.status === 'REJECTED' ? 'bg-red-500' : 'bg-amber-400'
                }`} />
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-2 mt-1.5 items-center text-xs text-muted-foreground">
                    ...
                    <span>·</span>
                    <span>{q.section}</span>
                    <span className={`px-1.5 py-0.5 rounded font-medium ${DIFFICULTY_COLORS[q.difficulty] ?? 'bg-slate-100 text-slate-600'}`}>
                      {q.difficulty}
                    </span>
                    <span className={`px-1.5 py-0.5 rounded font-medium ${STATUS_COLORS[q.status] ?? ''}`}>
                      {q.status}
                    </span>
                  </div>
                  ...
                  {expanded === q.id && (
                    <div className="mt-3 space-y-2 text-sm bg-muted/40 rounded-lg p-3 border">
                      ...
                      {q.explanation && (
                        <div>
                          <span className="font-semibold text-xs uppercase tracking-wide text-muted-foreground">Explanation</span>
