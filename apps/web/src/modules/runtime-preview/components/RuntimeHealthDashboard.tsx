                ...
                <tbody>
                  {buildsData.builds.map((build: any) => (
                    <React.Fragment key={build.id}>
                      <tr className={`border-b hover:bg-muted/30 ${build.status === 'FAILED' && build.errors ? 'border-b-0' : ''}`}>
                        <td className="px-4 py-3 font-mono text-xs">{build.id}</td>
                        <td className="px-4 py-3 font-mono text-xs">
                          <span className="truncate block max-w-[150px]" title={build.testId}>{build.testId}</span>
                        </td>
                        <td className="px-4 py-3">
                          <Badge 
                            variant={build.status === 'COMPLETED' ? 'default' : build.status === 'FAILED' ? 'destructive' : 'secondary'}
                            className={build.status === 'COMPLETED' ? 'bg-green-600 hover:bg-green-700' : ''}
                          >
                            {build.status}
                          </Badge>
                        </td>
                        ...
                        <tr className="border-b bg-red-50/30">
                          <td colSpan={5} className="px-4 py-2 text-xs text-red-600">
                            ...
                            {typeof build.errors === 'string' 
                              ? build.errors 
                              : build.errors.error || build.errors.message || JSON.stringify(build.errors)}
                          </td>
                        </tr>
                      )}
                    </React.Fragment>
