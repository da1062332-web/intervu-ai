...
function getActivityConfig(type: string) {
  switch (type.toLowerCase()) {
    ...
    case 'assessment_completed':
      return {
        icon: <FileText className='size-3.5 text-blue-600 dark:text-blue-400' />,
        bg: 'bg-blue-100 dark:bg-blue-900/30',
        border: 'border-blue-200 dark:border-blue-800',
      };
    case 'system':
      return {
        icon: <Settings className='size-3.5 text-amber-600 dark:text-amber-400' />,
        bg: 'bg-amber-100 dark:bg-amber-900/30',
        border: 'border-amber-200 dark:border-amber-800',
      };
    case 'user':
      return {
        icon: <User className='size-3.5 text-emerald-600 dark:text-emerald-400' />,
            ...
            {data.map((activity, index) => {
              ...
              return (
                <li key={index} className='relative pl-7 group'>
                  ...
                  <span
                    className={cn(
                      'absolute left-[-1.1rem] top-0.5 flex size-7 items-center justify-center rounded-full border shadow-sm transition-transform group-hover:scale-110',
                      config.bg,
                      config.border,
                    )}
                  >
