...
describe("Chaos & Pipeline Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    pipelineQueue = new Queue("chaos-test-queue", {
  ...
  it("Scenario A: Mixed Workload (100 valid, 20 invalid, 10 AI failures, 5 queue failures)", async () => {
    ...
    pipelineWorker = new Worker(
      ...
      async (job: Job<QueuePayload>) => {
        ...
        if (
          job.data.payload.type === "generation" &&
          job.data.payload.testId === "queue_fail"
        ) {
        ...
        if (
          payload.payload.type === "generation" &&
          payload.payload.testId === "ai_fail"
        ) {
      ...
      jobs.push({
        ...
        data: {
          ...
          payload: {
            type: "generation",
            testId: `valid_${i}`,
            data: { topic: "React", count: 1, difficulty: "beginner" },
          },
        },
      });
    }
