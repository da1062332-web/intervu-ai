...
describe("Analytics & Topic Mastery integration E2E", () => {
  it("should calculate accuracy values and transform them to Topic Mastery levels", async () => {
    ...
    const analyticsService = moduleFixture.get<PerformanceAnalyticsService>(
      PerformanceAnalyticsService,
    );
    const masteryService =
      moduleFixture.get<TopicMasteryService>(TopicMasteryService);
    ...
    const analytics = analyticsService.calculateAnalytics(
      evalResults,
      questions,
    );
    ...
    const masteryLevels = masteryService.calculateTopicMastery(
