...
describe('UsersService', () => {
  ...
  describe('getSessions', () => {
    it('should return mapped active sessions', async () => {
      ...
      expect(sessionRepository.findActiveSessionsByUserId).toHaveBeenCalledWith('user-1');
  ...
  describe('terminateSession', () => {
    it('should call repository to delete session', async () => {
      ...
      expect(sessionRepository.deleteSession).toHaveBeenCalledWith('session-1', 'user-1');
    });

    it('should throw NotFoundException if repository delete fails', async () => {
      jest.spyOn(sessionRepository, 'deleteSession').mockRejectedValue(new Error('Prisma error'));
  ...
  describe('terminateAllOtherSessions', () => {
    it('should terminate other sessions and tokens when currentSessionId is provided', async () => {
      ...
      expect(sessionRepository.deleteOtherSessions).toHaveBeenCalledWith('user-1', 'session-1');
      expect(sessionRepository.revokeOtherRefreshTokens).toHaveBeenCalledWith('user-1', 'session-1');
    });
    ...
    it('should terminate all sessions and tokens when currentSessionId is not provided', async () => {
      ...
      expect(sessionRepository.deleteAllSessionsByUserId).toHaveBeenCalledWith('user-1');
      expect(sessionRepository.revokeAllRefreshTokensByUserId).toHaveBeenCalledWith('user-1');
    });
  });
});
