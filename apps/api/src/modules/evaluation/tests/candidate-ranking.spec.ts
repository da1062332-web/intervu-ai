...
describe("CandidateRankingService", () => {
  ...
  it("should calculate correct rankings and percentiles across all cohorts", async () => {
    ...
    expect(result.assessment).toEqual({ rank: 3, totalCandidates: 10, percentile: 75 });
    expect(result.organization).toEqual({ rank: 5, totalCandidates: 20, percentile: 75 });
    expect(result.batch).toEqual({ rank: 1, totalCandidates: 5, percentile: 90 });
