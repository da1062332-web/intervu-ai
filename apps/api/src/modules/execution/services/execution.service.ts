...
import { SubmitExecutionDto, ExecutionResultDto, SubmitExecutionResponseDto } from "@/modules/execution/dto";
...
export class ExecutionService {
  ...
  async submitAnswers(dto: SubmitExecutionDto): Promise<SubmitExecutionResponseDto> {
    ...
    for (const ans of dto.answers) {
      if (questionIds.has(ans.questionId)) {
        this.logger.warn("Validation failed: duplicate questionId", { questionId: ans.questionId });
        throw new BadRequestException(`Duplicate questionId found: ${ans.questionId}`);
      }
      questionIds.add(ans.questionId);
    }

    // 4. Detect duplicate submission
    const alreadySubmitted = await this.redisCacheService.hasExecutionForTest(dto.testId);
    if (alreadySubmitted) {
      this.logger.warn("Duplicate submission detected", { testId: dto.testId });
      throw new ConflictException("Execution for this test has already been submitted");
    }
    ...
    // 7. Persist to Redis
    const isRedisAvailable = await this.redisCacheService.setExecution(executionId, executionData);
    if (!isRedisAvailable) {
      this.logger.error("Failed to persist execution due to Redis unavailability", null, { executionId });
    ...
    await this.redisCacheService.setExecutionForTest(dto.testId, executionId, 30 * 24 * 3600);

    this.logger.info("Execution submitted successfully", { executionId, testId: dto.testId });
  ...
  async getExecutionResult(id: string): Promise<ExecutionResultDto> {
    ...
    const execution = await this.redisCacheService.getExecution<ExecutionResultDto>(id);
