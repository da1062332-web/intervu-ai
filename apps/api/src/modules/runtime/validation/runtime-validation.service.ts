  ...
  validate(test: RuntimeTestDto): RuntimeValidationResultDto {
    ...
    test.sections?.forEach((section, index) => {
      ...
      if (section.duration < 0) {
        errors.push(`Negative Duration in section ${section.sectionId || index}`);
      }

      if (!section.questions || section.questions.length === 0) {
        errors.push(`Empty Sections are not allowed (Section ${section.sectionId || index})`);
      } else {
        if (section.questionCount !== section.questions.length) {
          errors.push(`Question Count mismatch in section ${section.sectionId || index}`);
        }

        section.questions.forEach((q, qIndex) => {
          if (!q.questionId) {
            errors.push(`Missing Questions: Question ID is missing in section ${section.sectionId || index} at index ${qIndex}`);
          } else {
          ...
          if (!q.questionType) {
            errors.push(`Invalid Question Types: Question ${q.questionId || qIndex} is missing a type`);
          }

          if (q.questionType === 'MULTIPLE_CHOICE' && (!q.options || q.options.length === 0)) {
            errors.push(`Missing Options for multiple choice question ${q.questionId}`);
          }
        });
      }
    });
