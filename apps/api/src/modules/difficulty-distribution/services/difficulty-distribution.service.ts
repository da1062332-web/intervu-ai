...
import { UpdateDifficultyDistributionDto, DifficultyDistributionResponse } from '@intervu/shared';

@Injectable()
export class DifficultyDistributionService {
  constructor(private readonly repository: DifficultyDistributionRepository) {}

  async getDifficultyDistribution(examConfigId: string): Promise<DifficultyDistributionResponse | null> {
    const distribution = await this.repository.findByConfigId(examConfigId);
    if (!distribution) return null;
    return this.mapToResponse(distribution);
  }

  async updateDifficultyDistribution(examConfigId: string, dto: UpdateDifficultyDistributionDto): Promise<DifficultyDistributionResponse> {
    // Validate business rules
    if (dto.easyCount < 0 || dto.mediumCount < 0 || dto.hardCount < 0) {
      throw new BadRequestException({ code: 'INVALID_DISTRIBUTION', message: 'Question counts cannot be negative' });
    }

    const total = dto.easyCount + dto.mediumCount + dto.hardCount;
    if (total <= 0) {
      throw new BadRequestException({ code: 'INVALID_DISTRIBUTION', message: 'At least one question must exist.' });
    }
  ...
  private mapToResponse(distribution: any): DifficultyDistributionResponse {
    return {
      id: distribution.id,
