...
export class GenerationConfig {
  ...
  @ApiProperty({ example: { easy: 0.5, medium: 0.7, hard: 0.9 }, description: 'Temperature presets per difficulty' })
...
export class ValidationConfig {
  ...
  @ApiProperty({ example: 5, description: 'Maximum validation error threshold' })
  maxValidationErrors!: number;

  @ApiProperty({ example: ['multiple-choice', 'coding', 'free-text'], description: 'Supported question types' })
  allowedTypes!: string[];

  @ApiProperty({ example: false, description: 'Allow undocumented/unknown properties' })
  allowUnknownFields!: boolean;
}

export class QueueConfig {
  @ApiProperty({ example: { generation: 5, evaluation: 2, analytics: 10 }, description: 'Concurrency count by queue name' })
  concurrency!: Record<string, number>;

  @ApiProperty({ example: 30000, description: 'Job execution timeout in milliseconds' })
  jobTimeoutMs!: number;
...
export class EnvironmentFlags {
  @ApiProperty({ example: false, description: 'System-wide maintenance mode flag' })
  maintenanceMode!: boolean;

  @ApiProperty({ example: true, description: 'Enable background processing worker' })
...
export class SystemConfigDto {
  @ApiProperty({ type: [DifficultyConfig], description: 'Configured difficulty levels' })
  ...
  @ApiProperty({ type: ValidationConfig, description: 'Input/output validation rules' })
  validationRules!: ValidationConfig;

  @ApiProperty({ type: QueueConfig, description: 'Queue background processing parameters' })
  queueSettings!: QueueConfig;

  @ApiProperty({ type: EnvironmentFlags, description: 'Global environment configuration toggles' })
