...
describe('SystemConfigService', () => {
  ...
  const mockConfigDto = {
    difficultyLevels: [
      { id: 'easy', name: 'Easy', timeLimitSeconds: 60, weight: 1, isActive: true, passingScore: 60 },
      { id: 'medium', name: 'Medium', timeLimitSeconds: 120, weight: 2, isActive: true, passingScore: 70 },
      { id: 'hard', name: 'Hard', timeLimitSeconds: 180, weight: 3, isActive: true, passingScore: 80 }
    ],
    generationRules: {
      defaultModel: 'gpt-4',
      temperature: 0.7,
  ...
  describe('getSystemConfig', () => {
    ...
    it('should fetch from repository and populate cache on cache miss', async () => {
      ...
      expect(cacheService.set).toHaveBeenCalledWith('config:system', mockConfigDto, { ttl: 3600 });
      expect(result).toEqual(mockConfigDto);
    });
  });
