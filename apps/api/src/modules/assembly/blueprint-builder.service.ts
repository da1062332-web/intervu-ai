...
export class BlueprintBuilderService {
  ...
  async generateBlueprint(configId: string): Promise<BlueprintDto> {
    ...
    if (!config.sections || config.sections.length === 0) {
      throw new BadRequestException(`Test configuration ${configId} has no sections defined`);
    }

    const sections: BlueprintSectionDto[] = config.sections.map((section: TestSection) => ({
