...
export class RecommendationsService {
  ...
  async getRecommendations(userId: string, evaluationId: string): Promise<RecommendationResponseDto[]> {
    // Ownership validation via resultsService
    await this.resultsService.getResultDetails(userId, evaluationId);

    const recommendations = await this.recommendationRepository.findByEvaluationId(evaluationId);
  ...
  async getHighPriorityRecommendations(userId: string, evaluationId: string): Promise<RecommendationResponseDto[]> {
