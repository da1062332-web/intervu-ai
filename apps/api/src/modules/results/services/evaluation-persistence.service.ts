...
export const StoreEvaluationOutcomeSchema = z.object({
  evaluation: z.object({
  ...
  (data) => (data.evaluation.testId && !data.evaluation.testInstanceId) || (!data.evaluation.testId && data.evaluation.testInstanceId),
  {
    message: "Either testId or testInstanceId must be provided, but not both.",
    path: ["evaluation", "testId"]
  }
);

export type StoreEvaluationOutcomeInput = z.infer<typeof StoreEvaluationOutcomeSchema>;
...
export class EvaluationPersistenceService {
   ...
   */
  async storeEvaluationOutcome(data: StoreEvaluationOutcomeInput): Promise<void> {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      const evalResult = await this.evaluationResultRepo.createEvaluationWithNested(
        {
