...
import { CreateConceptMappingDto, UpdateConceptMappingDto } from "@intervu/shared";
...
export class ConceptMappingService {
  ...
  async createConcept(topicId: string, dto: CreateConceptMappingDto) {
    ...
    if (!topic) {
      throw new BadRequestException(`Topic ID "${topicId}" does not exist in the Topic Registry`);
    }

    const existing = await this.repository.findByTopicAndCode(topicId, dto.conceptCode);
    if (existing) {
      throw new ConflictException(`Concept code ${dto.conceptCode} already exists for topic ${topicId}`);
    }
  ...
  async updateConcept(conceptId: string, dto: UpdateConceptMappingDto) {
    ...
    if (dto.conceptCode !== undefined && dto.conceptCode !== concept.conceptCode) {
      const existing = await this.repository.findByTopicAndCode(concept.topicId, dto.conceptCode);
      if (existing && existing.id !== conceptId) {
        throw new ConflictException(`Concept code ${dto.conceptCode} already exists for topic ${concept.topicId}`);
      }
    }
