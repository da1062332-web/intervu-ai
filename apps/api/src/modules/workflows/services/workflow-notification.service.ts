...
export class WorkflowNotificationService {
  ...
  handleConfigurationCompleted(event: ConfigurationCompletedEvent) {
    this.logger.log(
      `[Workflow] Configuration completed for exam ${event.examId}`,
    );
  }
  ...
  handleGenerationStarted(event: GenerationStartedEvent) {
    this.logger.log(
      `[Workflow] Question generation started for exam ${event.examId}`,
    );
  }
  ...
  handleAssemblyCompleted(event: AssemblyCompletedEvent) {
    this.logger.log(
      `[Workflow] Test assembly completed for exam ${event.examId}`,
    );
  }
  ...
  handlePublishingCompleted(event: PublishingCompletedEvent) {
    this.logger.log(
      `[Workflow] Test publishing completed for exam ${event.examId}`,
    );
  }
  ...
  handleWorkflowCompleted(event: WorkflowCompletedEvent) {
    this.logger.log(
      `[Workflow] Entire workflow completed successfully for exam ${event.examId}`,
    );
  }
  ...
  handleWorkflowFailed(event: WorkflowFailedEvent) {
    this.logger.error(
      `[Workflow] Workflow FAILED for exam ${event.examId}. Reason: ${event.reason}`,
    );
  }
  ...
  handleWorkflowRetry(event: WorkflowRetryEvent) {
    this.logger.log(
      `[Workflow] Retrying step ${event.step} for exam ${event.examId}`,
    );
  }
}
