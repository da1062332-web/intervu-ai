...
import { NextAction, StepStatus, WorkflowStatusDto } from '../dto/workflow-status.dto';
...
export class WorkflowStatusService {
  ...
  async aggregateStatus(workflow: ExamWorkflow): Promise<Omit<WorkflowStatusDto, 'nextAction' | 'currentStep' | 'completionPercentage' | 'status'>> {
    const examId = workflow.examId;

    // 1. Configuration Status
    const configStatus = await this.getConfigurationStatus(examId, workflow.currentStep);

    // 2. Generation Status
    const generationStatus = await this.getGenerationStatus(examId, workflow.currentStep);

    // 3. Review Status
    const reviewStatus = await this.getReviewStatus(examId, workflow.currentStep);

    // 4. Assembly Status
    const assemblyStatus = await this.getAssemblyStatus(examId, workflow.currentStep);

    // 5. Publishing Status (pass workflow status so COMPLETED workflows show publishing as done)
    const publishingStatus = await this.getPublishingStatus(examId, workflow.currentStep, workflow.status);
  ...
  private async getConfigurationStatus(examId: string, currentStep: WorkflowStep): Promise<StepStatus> {
    try {
      const config = await this.prisma.examConfig.findUnique({ where: { id: examId } });
      if (!config) {
  ...
  private async getGenerationStatus(examId: string, currentStep: WorkflowStep): Promise<StepStatus> {
    ...
    try {
      ...
      if (currentStep === WorkflowStep.QUESTION_GENERATION) {
        return this.buildStepStatus(questionCount > 0 ? 'COMPLETED' : 'IN_PROGRESS', questionCount > 0 ? 100 : 50);
      }
      return this.buildStepStatus(questionCount > 0 ? 'COMPLETED' : 'NOT_STARTED', questionCount > 0 ? 100 : 0);
    } catch (e) {
      return this.buildStepStatus('BLOCKED', 0, 'Service unavailable');
    }
  }

  private async getReviewStatus(examId: string, currentStep: WorkflowStep): Promise<StepStatus> {
    ...
    try {
      ...
      const progress = totalQuestions > 0 ? Math.floor((approvedQuestions / totalQuestions) * 100) : 0;
      
      if (currentStep === WorkflowStep.QUESTION_REVIEW) {
        return this.buildStepStatus(progress >= 100 ? 'COMPLETED' : 'IN_PROGRESS', progress);
      }
      
      return this.buildStepStatus(progress >= 100 ? 'COMPLETED' : 'NOT_STARTED', progress);
    } catch (e) {
      return this.buildStepStatus('BLOCKED', 0, 'Service unavailable');
    }
  }

  private async getAssemblyStatus(examId: string, currentStep: WorkflowStep): Promise<StepStatus> {
    ...
    if (currentStep === WorkflowStep.PUBLISHING || currentStep === WorkflowStep.COMPLETED) {
      return this.buildStepStatus('COMPLETED', 100);
    }
    ...
    try {
      ...
      return this.buildStepStatus(currentStep === WorkflowStep.ASSEMBLY ? 'IN_PROGRESS' : 'NOT_STARTED', 0);
    } catch (e) {
      return this.buildStepStatus('BLOCKED', 0, 'Service unavailable');
    }
  }

  private async getPublishingStatus(examId: string, currentStep: WorkflowStep, workflowStatus?: string): Promise<StepStatus> {
    if (this.isStepBefore(currentStep, WorkflowStep.PUBLISHING) && currentStep !== WorkflowStep.COMPLETED) {
      return this.buildStepStatus('NOT_STARTED', 0);
    }
    
    // If the whole workflow is COMPLETED, publishing is definitely done
    if (workflowStatus === 'COMPLETED' || currentStep === WorkflowStep.COMPLETED) {
      return this.buildStepStatus('COMPLETED', 100);
    }
    ...
    try {
      ...
      return this.buildStepStatus(currentStep === WorkflowStep.PUBLISHING ? 'IN_PROGRESS' : 'NOT_STARTED', 0);
    } catch (e) {
      return this.buildStepStatus('BLOCKED', 0, 'Service unavailable');
    }
  }
