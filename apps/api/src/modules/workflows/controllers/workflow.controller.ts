...
import { ApiTags, ApiOperation, ApiOkResponse, ApiCreatedResponse, ApiBadRequestResponse, ApiNotFoundResponse, ApiConflictResponse, ApiBearerAuth } from '@nestjs/swagger';
...
import { CreateWorkflowDto, RollbackDto, RetryWorkflowDto, BulkWorkflowDto, WorkflowResponseDto } from '../dto/workflow.dto';
...
export class WorkflowController {
  ...
  @ApiOperation({ summary: 'Workflow module health check for operational monitoring' })
  ...
  @ApiBadRequestResponse({ description: 'ExamConfig not found or workflow already exists' })
  ...
  @Get(':examId/overview')
  @ApiOperation({ summary: 'Get overview data for a workflow (exam config, question stats)' })
  ...
  @ApiConflictResponse({ description: 'Invalid transition or concurrent modification' })
  advanceWorkflow(@Param('examId') examId: string, @CurrentUser() user: AuthUser) {
  ...
  startGeneration(@Param('examId') examId: string, @CurrentUser() user: AuthUser) {
  ...
  startAssembly(@Param('examId') examId: string, @CurrentUser() user: AuthUser) {
  ...
  rollbackWorkflow(@Param('examId') examId: string, @Body() dto: RollbackDto, @CurrentUser() user: AuthUser) {
    return this.facade.rollbackWorkflow(examId, dto.reason, user.id);
  }

  @Post(':examId/publish')
  @ApiOperation({ summary: 'Publish the assembled test and complete the workflow' })
  @ApiConflictResponse({ description: 'Workflow not in a publishable state' })
  publishWorkflow(@Param('examId') examId: string, @CurrentUser() user: AuthUser) {
    return this.facade.publishWorkflow(examId, user.id);
  }

  @Post(':examId/retry')
  @ApiOperation({ summary: 'Retry a failed workflow stage without full rollback' })
  @ApiBadRequestResponse({ description: 'Stage is not retryable' })
  retryWorkflow(@Param('examId') examId: string, @Body() dto: RetryWorkflowDto, @CurrentUser() user: AuthUser) {
    return this.facade.retryWorkflow(examId, dto.step, user.id);
  }

  @Post('bulk/:action')
  @ApiOperation({ summary: 'Bulk workflow operations (publish, archive, retry)' })
  bulkOperation(@Param('action') action: string, @Body() dto: BulkWorkflowDto, @CurrentUser() user: AuthUser) {
    switch (action) {
      case 'publish': return this.facade.bulkPublish(dto.examIds, user.id);
      case 'archive': return this.facade.bulkArchive(dto.examIds, user.id);
      case 'retry': return this.facade.bulkRetry(dto.examIds, dto.step!, user.id);
  ...
  @ApiCreatedResponse({ description: 'Bulk operation result' })
  bulkRetry(@Body() dto: BulkWorkflowDto & { step: WorkflowStep }, @CurrentUser() user: AuthUser) {
