...
export class ExamConfigService {
  ...
  async create(
    ...
    if (existing) {
      throw new ConflictException(`Exam configuration with code "${dto.code}" already exists`);
    }
  ...
  async findAll(): Promise<ExamConfig[]> {
    return this.examConfigRepository.findAll({ isActive: true, isArchived: false });
  }
