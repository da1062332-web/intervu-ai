...
export class TemplateRendererService {
   ...
   */
  render(template: string, payload: Record<string, unknown>): { renderedOutput: string; resolvedVariables: Record<string, unknown> } {
    if (!template) {
      return { renderedOutput: "", resolvedVariables: {} };
    }
    const resolvedVariables: Record<string, unknown> = {};
    const renderedOutput = template.replace(/\{\{\s*([\w]+)\s*\}\}/g, (match, key) => {
