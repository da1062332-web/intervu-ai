...
describe("StrategyDraftingService", () => {
  ...
  describe("draftStrategy", () => {
    it("should successfully draft a strategy from a valid prompt", async () => {
      const mockLLMResponse = JSON.stringify({
        ...
        derivedVariables: [{ name: "total", expression: "price * quantity" }],
        constraints: [{ rule: "total % 100 == 0", severity: "error" }],
    ...
    it("should handle LLM API errors gracefully", async () => {
      mockLLMAdapter.generate.mockRejectedValue(new Error("OpenAI API error"));
    ...
    it("should return validation warnings for weak strategies", async () => {
      ...
      expect(
        result.validationWarnings?.some((warning) =>
          warning.toLowerCase().includes("no variables"),
        ),
      ).toBe(true);
    });
    ...
    it("should accept a derived gcd expression in generated draft", async () => {
      const mockLLMResponse = JSON.stringify({
        ...
        derivedVariables: [
          {
            name: "gcdValue",
            expression: "gcd(rawSumNumerator, rawSumDenominator)",
          },
        ],
        constraints: [],
        notes: [],
      ...
      const result = await service.draftStrategy(
        "create a gcd derived variable draft",
      );

      expect(result.success).toBe(true);
      expect(result.data?.derivedVariables[0].expression).toBe(
  ...
  describe("normalizeStrategy", () => {
    ...
    it("should trim variable names", () => {
      const raw: any = {
        variables: [{ name: "  price  ", type: "number", min: 100, max: 500 }],
        derivedVariables: [],
        constraints: [],
        notes: [],
      };
  ...
  describe("collectWarnings", () => {
    ...
    it("should warn if no variables are defined", () => {
      ...
      expect(
        warnings.some((w) => w.toLowerCase().includes("no variables")),
      ).toBe(true);
    });
  ...
  describe("canonical validation", () => {
    it("rejects natural-language constraint rules", async () => {
      const mockLLMResponse = JSON.stringify({
        ...
        derivedVariables: [],
        constraints: [
          { rule: "avgRamMohan is an integer", severity: "critical" },
        ],
        notes: [],
      });
      ...
      await expect(service.draftStrategy("test")).rejects.toThrow(
        /invalid constraint definitions|undefined identifier/i,
      );
    });
  ...
  describe("buildDraftingPrompt", () => {
    ...
    it("should explicitly require variable domains to use type/min/max", () => {
      ...
      expect(systemPrompt).toContain(
        "Use 'variables' for base-domain requirements",
      );
    ...
    it("should explicitly forbid English prose in constraints.rule", () => {
      ...
      expect(systemPrompt).toContain(
        "Invalid examples: 'ages are integers between 10 and 70', 'avgRamMohan is an integer'",
      );
      expect(systemPrompt).toContain(
    ...
    it("should guide derived expressions to use only mathematical RHS values", () => {
      ...
      expect(systemPrompt).toContain(
        "Derived variables must use 'expression' only",
      );
      expect(systemPrompt).toContain(
        "Do NOT output 'avgRamMohan = (ram_age + mohan_age) / 2' inside 'expression'",
      );
  ...
  describe("Error Handling", () => {
    ...
    it("should handle empty derived variable expressions", async () => {
      ...
      expect(
        result.data?.derivedVariables.some((dv: any) => !dv.expression),
      ).toBe(false);
    });
  });
