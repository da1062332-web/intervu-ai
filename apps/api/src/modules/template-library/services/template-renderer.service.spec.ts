...
describe('TemplateRendererService', () => {
  ...
  it('should replace valid placeholders', () => {
    ...
    expect(result.renderedOutput).toBe('Correct answer is 42 because derived from formula');
    expect(result.resolvedVariables).toEqual({ answer: '42', explanation: 'derived from formula' });
  });
  ...
  it('should ignore missing payload keys', () => {
    ...
    const result = service.render(template, payload);
    expect(result.renderedOutput).toBe('Correct answer is 42 because {{explanation}}');
