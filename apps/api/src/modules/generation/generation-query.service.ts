...
export class GenerationQueryService {
  ...
  async getQuestionsByConcept(conceptKey: string, limit: number = 50, page: number = 1): Promise<GeneratedQuestion[]> {
    this.logger.debug(`Fetching questions for concept: ${conceptKey}`);
    return await this.questionPoolRepository.findQuestions({ conceptKey }, { limit, page });
  }

  async getQuestionsByDifficulty(difficultyLevel: DifficultyLevel, limit: number = 50, page: number = 1): Promise<GeneratedQuestion[]> {
    this.logger.debug(`Fetching questions for difficulty: ${difficultyLevel}`);
    return await this.questionPoolRepository.findQuestions({ difficultyLevel }, { limit, page });
  }
   ...
   */
  async getQuestionsForAssembly(filters: QuestionPoolFilter, requiredCount: number): Promise<GeneratedQuestion[]> {
    this.logger.debug(`Fetching ${requiredCount} randomized questions for assembly with filters: ${JSON.stringify(filters)}`);
    return await this.questionPoolRepository.findRandomizedSet(filters, requiredCount);
  }
