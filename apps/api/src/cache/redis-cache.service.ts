...
export class RedisCacheService {
  ...
  async get<T>(key: string, options?: CacheOptions): Promise<T | null> {
    if (!this.isAvailable()) {
      this.logger.debug('Redis unavailable — cache miss (degraded mode)', { key });
      return null;
    }
    try {
  ...
  async set<T>(key: string, value: T, options?: CacheOptions): Promise<boolean> {
    if (!this.isAvailable()) {
      this.logger.debug('Redis unavailable — cache set skipped (degraded mode)', { key });
      return false;
    }
    try {
      const redis = RedisConnectionManager.getInstance();
  ...
  async setTemplate<T>(templateId: string, data: T, ttl?: number): Promise<boolean> {
    return this.set<T>(templateId, data, { prefix: 'template', ttl: ttl ?? TTL.TEMPLATE_ITEM });
  }
  ...
  async setSystemTemplates<T>(data: T): Promise<boolean> {
    return this.set<T>('system', data, { prefix: 'template', ttl: TTL.TEMPLATE_SYSTEM });
  }
  ...
  async setTemplatesByDifficulty<T>(difficulty: string, data: T): Promise<boolean> {
    return this.set<T>(difficulty, data, { prefix: 'template:difficulty', ttl: TTL.TEMPLATE_DIFFICULTY });
  }
  ...
  async setTemplateList<T>(filterHash: string, data: T): Promise<boolean> {
    return this.set<T>(filterHash, data, { prefix: 'template:list', ttl: TTL.TEMPLATE_LIST });
  }
  ...
  async setGenerationResult<T>(jobId: string, data: T): Promise<boolean> {
    return this.set<T>(jobId, data, { prefix: 'generation', ttl: TTL.GENERATION_RESULT });
  }
}
