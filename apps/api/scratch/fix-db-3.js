...
async function run() {
  const qs = await prisma.generatedQuestion.findMany({ where: { id: { startsWith: 'mock-q-' } } });
  ...
  const qs2 = await prisma.question.findMany({ where: { id: { startsWith: 'mock-q-' } } });
  console.log('Mock questions in Question:', qs2.length);
  
  if (qs2.length > 0) {
    for (const q of qs2) {
      const md = (q.metadata || {});
      md.options = md.options || [{ id: 'opt1', text: 'A' }, { id: 'opt2', text: 'B' }];
  ...
  const qs3 = await prisma.assembledTestQuestion.findMany({ where: { questionId: { startsWith: 'mock-q-' } } });
  console.log('Mock questions in AssembledTestQuestion:', qs3.length);
  if (qs3.length > 0) {
    for (const q of qs3) {
      const snap = q.questionSnapshot || {};
      snap.options = [ { id: 'opt1', text: 'A' }, { id: 'opt2', text: 'B' } ];
