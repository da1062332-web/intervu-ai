...
_Note: Since the backend does not currently support these models, the frontend implementation of Concept -> Template mapping will mock the assignment API calls or use existing fallback endpoints (like updating `Template.conceptKey`) until the backend is updated as documented above._
