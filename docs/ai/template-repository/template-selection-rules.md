...
1. **Fallback 1: Concept Relaxation (Same Topic, Alternative Concept)**
   * Search for templates within the *same* `topicId` and `difficulty` that are associated with a closely related `conceptId` (defined in the Topic Registry's concept list).
