...
* **WAN Latency**: High network roundtrip latency to the database.
  * *Mitigation*: The app uses optimized single-roundtrip nested writes and raw SQL calculations to avoid query chaining.
* **Connection Pool Limitation**: Small database connection limit of 9.
  * *Mitigation*: Concurrent bulk operations are controlled via concurrency-limited batch queues to prevent pool timeouts.
