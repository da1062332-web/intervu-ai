...
export class RecommendationValidatorService {
   ...
   */
  validate(result: RecommendationResultDto): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    for (const rec of result.recommendations) {
      ...
      if (!rec.recommendationId || typeof rec.recommendationId !== "string" || rec.recommendationId.trim() === "") {
        errors.push("Recommendation ID is missing or empty.");
      } else {
        if (seenIds.has(rec.recommendationId)) {
          errors.push(`Duplicate recommendation ID found: ${rec.recommendationId}`);
        }
        seenIds.add(rec.recommendationId);
      }

      // Check skill
      if (!rec.skill || typeof rec.skill !== "string" || rec.skill.trim() === "") {
        ...
        if (seenSkills.has(rec.skill)) {
          errors.push(`Duplicate recommendation for skill/concept: ${rec.skill}`);
        }
      ...
      if (!rec.priority || !["HIGH", "MEDIUM", "LOW"].includes(rec.priority)) {
        errors.push(`Invalid priority value: ${rec.priority}. Must be HIGH, MEDIUM, or LOW.`);
      }
      ...
      // Check description
      if (!rec.description || typeof rec.description !== "string" || rec.description.trim() === "") {
