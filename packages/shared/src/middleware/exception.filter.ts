...
export class GlobalExceptionFilter implements ExceptionFilter {
    ...
    if (exception && typeof exception === 'object' && 'code' in exception && typeof (exception as Record<string, unknown>).code === 'string') {
       ...
       if (code === 'VALIDATION_ERROR' || code === 'CONTRACT_ERROR') status = HttpStatus.BAD_REQUEST;
    ...
    else if (exception && typeof exception === 'object' && exception.constructor?.name?.startsWith('Prisma')) {
    ...
    // Support for ZodError
    else if (exception && typeof exception === 'object' && exception.constructor?.name === 'ZodError') {
      ...
      if (typeof res === 'object' && res !== null && 'success' in res && (res as Record<string, unknown>).success === false && 'error' in res) {
        return response.status(status).json(res);
      }

      code = typeof res === 'object' && res !== null && 'error' in res ? String((res as Record<string, unknown>).error) : 'HTTP_ERROR';
      ...
      message = typeof res === 'object' && res !== null && 'message' in res ? String((res as Record<string, unknown>).message) : exception.message;
      details = res;
