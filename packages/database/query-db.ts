...
async function runQueries() {
  try {
    ...
    const countResult = await prisma.$queryRaw`SELECT COUNT(*) FROM "GeneratedQuestion";`;
    // Prisma raw count usually returns BigInt, so we convert to string for safe logging
    console.log(JSON.stringify(countResult, (key, value) =>
    ...
    const duplicateResult = await prisma.$queryRaw`
      ...
      HAVING COUNT(*) > 1;
    `;
    console.log(JSON.stringify(duplicateResult, (key, value) =>
      typeof value === 'bigint' ? value.toString() : value
    , 2));
