...
describe("Day 4: Execution Persistence & Submission Locking", () => {
  ...
  test("SUB-001: should enforce atomicity with a flat-array transaction for autosave", async () => {
    ...
    mockPrisma.candidateAnswer.upsert = vi.fn().mockReturnValue("upsert_answer");
    ...
    await executionPersistenceRepo.saveAnswerAndState(
      ...
      { testInstanceId: "inst_1", currentQuestionIndex: 2, remainingTimeSeconds: 500 }
    );
    ...
    expect(mockPrisma.$transaction).toHaveBeenCalledWith(["upsert_answer", "upsert_state"]);
  });

  test("SUB-002: should lazily create a PENDING submission record", async () => {
    mockPrisma.submission.upsert = vi.fn().mockResolvedValue({ status: SubmissionStatus.PENDING });
  ...
  test("SUB-003: should lock status to SUBMITTED and stamp date", async () => {
    mockPrisma.submission.update = vi.fn().mockResolvedValue({ status: SubmissionStatus.SUBMITTED });
    await submissionRepo.updateStatus("inst_1", SubmissionStatus.SUBMITTED, "hash_123");
