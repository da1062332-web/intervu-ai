...
async function runDay2Tests() {
  ...
  try {
    ...
    if (!template)
      throw new Error(
        "Base template not found! Make sure Day 1 seed is loaded.",
      );
    ...
    console.log(
      "Running POOL-008: Batch insert testing with skipDuplicates...",
    );
    const batchData = [
    ...
    assert(
      insertedCount === 2,
      `Should insert 2 distinct records, got ${insertedCount}`,
    );
    console.log("✅ POOL-008 passed.");

    // POOL-003 to 007: Concept filtering, pagination, and counts
    console.log(
      "Running POOL-003 to 007: Retrieval, Filtering, Counts, Pagination...",
    );
    ...
    assert(
      uniqueIds.size === 10,
      "All returned IDs must be completely unique, no duplicates",
    );
