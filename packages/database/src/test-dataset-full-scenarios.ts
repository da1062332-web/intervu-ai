...
async function runDatasetFullScenariosTest() {
  console.log(
    "🚀 Starting Full Dataset End-to-End Scenarios Integration Test...\n",
  );
  ...
  try {
    ...
    console.log(
      `📌 Using Concept: "${concept?.name || "N/A"}" (${concept?.id || "N/A"})\n`,
    );
    ...
    console.log(
      `✅ Dataset created successfully: ID=${dataset.id}, Name="${dataset.name}"`,
    );
    console.log(
      `   TopicId: ${dataset.topicId}, ConceptId: ${dataset.conceptId}\n`,
    );
    ...
    const isAnswerInOptions = validOptions.some(
      (o) => o.trim().toLowerCase() === validAnswer.trim().toLowerCase(),
    );
    if (!isAnswerInOptions) {
      throw new Error(
        `Validation Error: Correct answer "${validAnswer}" is not in options [${validOptions.join(", ")}]`,
      );
    }
    ...
    const singleItem = await prisma.datasetItem.create({
      data: {
        ...
        answer: validAnswer,
        explanation:
          "Unsorted arrays require checking each element sequentially.",
        difficulty: "MEDIUM",
    ...
    const matchCheck = invalidOptions.some(
      (o) => o.trim().toLowerCase() === invalidAnswer.trim().toLowerCase(),
    );
    if (!matchCheck) {
      console.log(`🛡️ Validation Block Triggered Correctly!`);
      console.log(
        `   Attempted Answer "${invalidAnswer}" was blocked because it's not in [${invalidOptions.join(", ")}]\n`,
      );
    } else {
      throw new Error(
        "FAILED: Mismatched answer was not caught by validation logic.",
      );
    }
    ...
    console.log(
      `✅ Bulk Items inserted successfully: Count=${bulkResult.count}\n`,
    );
    ...
    const updatedItem = await prisma.datasetItem.update({
      ...
      data: {
        questionText:
          "Updated: What is linear search time complexity in an unsorted array?",
        difficulty: "HARD",
        answer: "O(N)",
      },
    });
    ...
    // -------------------------------------------------------------
    console.log(
      "--- SCENARIO 6: Retrieve Dataset with Items & Normalized Relation ---",
    );
