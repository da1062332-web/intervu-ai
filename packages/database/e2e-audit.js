...
async function getToken() {
  const r = await fetch(`${BASE}/auth/login`, {
    ...
    body: JSON.stringify({ email: 'admin@intervu.ai', password: 'Intervu123!' }),
  });
...
async function main() {
  ...
  if (demoTemplate) {
    ...
    info(`Demo template not found, using first: "${allTemplates[0]?.name}" (${templateId})`);
  }

  if (!templateId) { fail('No template ID — cannot continue.'); process.exit(1); }
  ...
  if (detailRes.status === 200 && detailRes.body.success) {
    pass(`GET /templates/${templateId} → 200. Name: "${detailRes.body.data?.name}"`);
  } else {
  ...
  console.log('\nPHASE 3 — GET SOLUTION TEMPLATE');
  const solGetRes = await api(token, 'GET', `/templates/${templateId}/solution`);
  if (solGetRes.status === 200) {
    const hasData = solGetRes.body.data !== null;
    pass(`GET /solution → 200. Data ${hasData ? 'exists' : 'is null (none saved yet)'}.`);
  } else {
    fail(`GET /solution → ${solGetRes.status}: ${JSON.stringify(solGetRes.body)}`);
  }

  // ── PHASE 4 — Template Variables ────────────────────────
  console.log('\nPHASE 4 — TEMPLATE VARIABLES');
  const varListRes = await api(token, 'GET', `/templates/${templateId}/variables`);
  let variables = varListRes.body.data ?? [];

  if (varListRes.status === 200 && variables.length > 0) {
    pass(`Variables already exist: ${variables.map(v => v.variableName).join(', ')}`);
  } else {
    // Create variables via API so the template is fully functional
    info('No variables found — creating test variables via API (POST /variables)...');
    const createVarRes1 = await api(token, 'POST', `/templates/${templateId}/variables`, {
      variableName: 'answer',
      variableType: 'STRING',
      required: true,
      defaultValue: '',
    });
    const createVarRes2 = await api(token, 'POST', `/templates/${templateId}/variables`, {
    ...
    if (createVarRes1.status === 201 && createVarRes2.status === 201) {
      ...
      fail(`Failed to create variables. answer→${createVarRes1.status}, explanation→${createVarRes2.status}`);
  ...
  if (!existingSol) {
    ...
    if (saveRes.status === 201 && saveRes.body.success) {
      ...
      fail(`POST /solution → ${saveRes.status}: ${JSON.stringify(saveRes.body)}`);
    }
  } else {
    ...
    if (saveRes.status === 200 && saveRes.body.success) {
      ...
      fail(`PATCH /solution → ${saveRes.status}: ${JSON.stringify(saveRes.body)}`);
    }
  }

  // Verify persistence
  const solVerifyRes = await api(token, 'GET', `/templates/${templateId}/solution`);
  if (solVerifyRes.body.data?.solutionTemplate?.includes('{{answer}}')) {
    pass('Persistence verified — solution template saved and retrieved correctly.');
  } else {
    fail(`Persistence check failed. Got: "${solVerifyRes.body.data?.solutionTemplate}"`);
  }

  // ── PHASE 6 — Valid Preview ──────────────────────────────
  console.log('\nPHASE 6 — VALID TEMPLATE PREVIEW');
  const previewRes = await api(token, 'POST', `/templates/${templateId}/preview`, {
  ...
  if (previewRes.status === 201 && previewRes.body.success) {
    ...
    fail(`POST /preview → ${previewRes.status}: ${JSON.stringify(previewRes.body)}`);
  }

  // ── PHASE 7 — Invalid Placeholder ───────────────────────
  console.log('\nPHASE 7 — INVALID PLACEHOLDER PREVIEW');
  const invalidPreviewRes = await api(token, 'POST', `/templates/${templateId}/preview`, {
    previewPayload: { answer: '42' },
    solutionTemplate: 'Correct answer is {{invalid_variable}}',
  });
  const invalidValidation = invalidPreviewRes.body.data?.previewResult?.validation;
  const errorUnknown = invalidPreviewRes.body.error?.details?.unknownVariables;
  if (!invalidValidation?.valid && invalidValidation?.unknownVariables?.includes('invalid_variable')) {
    pass(`Invalid variable detected in previewResult — unknownVariables: ["invalid_variable"]`);
  } else if (errorUnknown?.includes('invalid_variable')) {
    pass(`API rejected template — error.unknownVariables: ["invalid_variable"]`);
  } else if (invalidPreviewRes.body.error) {
    pass(`API rejected invalid template: ${JSON.stringify(invalidPreviewRes.body.error)}`);
  } else {
    fail(`Invalid variable NOT caught. Got: ${JSON.stringify(invalidPreviewRes.body)}`);
  }
  ...
  if (latestRes.status === 200 && latestRes.body.success) {
    ...
    pass(`GET /preview → 200. Preview history ${hasHistory ? 'exists' : 'is null (none saved)'}.`);
    if (hasHistory) {
      info(`Last preview payload: ${JSON.stringify(latestRes.body.data?.previewPayload)}`);
    }
  } else {
    fail(`GET /preview → ${latestRes.status}: ${JSON.stringify(latestRes.body)}`);
  }
